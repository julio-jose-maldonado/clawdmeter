...
- No unit tests/linters. CI (`.github/workflows/ci.yml`) runs on every push/PR: compiles the firmware with `arduino-cli` (ESP32-S3, FQBN `esp32:esp32:esp32s3:PSRAM=opi,FlashSize=16M`) via `arduino/compile-sketches`, and `node --check`s the proxy JS + the PWA inline scripts.
