...
- Requires `.env` (copy from `.env.example`): `PROXY_PORT` (default 3456), `CDP_PORT` (default 9222), `WARN_THRESHOLD` (default 80 — % "red zone" for the history stats).
...
- `public/` — PWA dashboard (vanilla HTML/JS, manifest, service worker) served statically. Shows the live device plus a history view (canvas chart + stat cards, responsive row/column).

### firmware/Clawdmeter/ (Arduino, ESP32-S3)

Arduino IDE concatenates all `.ino` files in the folder into one compilation unit — functions defined in one file are callable from another without headers. All globals, structs (`Config`, `UsageData`), color/layout `#define`s, and `setup()/loop()` live in `Clawdmeter.ino`; the other files (`config`, `colors`, `alerts`, `display`, `screen_usage`, `screen_trend`, `screen_clock`, `screen_weather`, `network`, `touch`, `weather`, `webconfig`) only define functions against those globals. `display.ino` holds the common UI (boot messages, header, footer, `drawScreen()` dispatch); each screen's drawing lives in its own `screen_*.ino`.
...
- LEDs (`colors.ino`): the onboard WS2812 (GPIO38) runs a smooth random ambient color cycle (`tickAmbientLed`); an external chain of 3 WS2812B (Adafruit NeoPixel on `EXT_LED_PIN`, default GPIO2) shows **state, not raw usage**: LED0=5h and LED1=7d show the **projection** (green=safe/flat, amber=rising-but-resets-in-time, red=will hit the limit before reset), LED2=extra shows **system health** (green=fresh, amber=stale, red=down/no session). `updateUsageLeds()` sets target hues from the projection embedded in `/api/usage`; `tickExtLeds()` eases each LED toward its target by HSV hue (smooth green→amber→red, no abrupt jumps). The external strip needs 5V power and common ground.
