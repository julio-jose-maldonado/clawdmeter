...
- No unit tests (small project). CI (`.github/workflows/ci.yml`) runs on every push/PR: compiles the firmware with `arduino-cli` (ESP32-S3, FQBN `esp32:esp32:esp32s3:PSRAM=opi,FlashSize=16M`) via `arduino/compile-sketches`; for the proxy it `node --check`s the JS + PWA inline scripts and lints with **oxlint** (`npx oxlint server.js lib`). Local lint: `pnpm dlx oxlint server.js lib` (npm/npx may hit a private registry on this machine; pnpm works).
...
- `lib/config.js` — SQLite (`proxy/data/config.db`, single-row JSON) = source of truth for all **behavior** settings (brightness, refresh, thresholds, weather, night-dim, buzzer, timezone). NOT the bootstrap: WiFi + proxy IP/port stay local on the ESP32. Inputs validated/coerced per a field `SCHEMA`. Edited from the config view **embedded in the dashboard** (`index.html`, CONFIG button; `public/config.html` still exists as a standalone fallback — both use the same endpoints and share the `cfgAuth` sessionStorage key) via `POST /api/config`; the ESP32 fetches `GET /api/config` on boot and caches it to NVS. The page's own login lives in the same config: `config_user` + `config_pass` (bcrypt-hashed, never returned by the API, default `admin`/`clawdmeter`, changeable from the page's Seguridad panel). `/api/config` is Basic-Auth gated by those (no `WWW-Authenticate` header so the browser doesn't show its native dialog — the page has its own login form).
- `lib/status.js` — Claude service status from status.claude.com (Statuspage API, public/no-auth/no-Cloudflare → plain Node `fetch`, NOT via the browser). Watches 3 components (claude.ai, Claude Code, Claude API) by Statuspage id with name-prefix fallback, plus unresolved incidents affecting them. 2.5-min cache; on fetch failure keeps last data and flags `stale`. Served at `GET /api/status`; the PWA renders it as colored chips (gray when stale) + incident line.
...
- `public/` — PWA dashboard (vanilla HTML/JS, manifest, service worker) served statically. Single-console layout ("bezel" replicating the physical device: scanlines + 3 bezel LEDs mirroring the WS2812s): gauges (5h/7d/extra) | trend chart (canvas, 5h/24h/7d) | service status + summary. Header chips: LIGHT/DARK theme and FULL/MINI widget mode (both persisted in localStorage) and CONFIG (embedded config view with login). `public/config.html` remains as a standalone config fallback. No external fonts/assets on purpose: must work offline on the LAN.
